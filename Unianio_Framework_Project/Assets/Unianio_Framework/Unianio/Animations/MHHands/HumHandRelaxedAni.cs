...
{
    public class HumHandRelaxedAni : BaseHumHandAni, IHandAni
    {
        IHandAni IHandAni.Set(IComplexHuman human, BodySide side, double seconds)
        {
            return Set(human, side, seconds);
        }
        public HumHandRelaxedAni Set(IComplexHuman human, BodySide side, double seconds = 1)
        {
            Init(human, side);
            RotFingerToLocal(FingerName.Thumb, 1, fw_dn_sd(10,10), up_sd(-10));
            RotFingerToLocal(FingerName.Index, 1, fw_dn_sd(10, 5), v3.up);
            RotFingerToLocal(FingerName.Middle, 1, fw_dn(10), v3.up);
            RotFingerToLocal(FingerName.Ring, 1, fw_dn_sd(10, -5), v3.up);
            RotFingerToLocal(FingerName.Pinky, 1, fw_dn_sd(10, -10), v3.up);

            RotFingerToLocal(FingerName.Thumb, 2, v3.forward, v3.up);
            RotFingerToLocal(FingerName.Index, 2, fw_dn(20), v3.up);
            RotFingerToLocal(FingerName.Middle, 2, fw_dn(20), v3.up);
            RotFingerToLocal(FingerName.Ring, 2, fw_dn(20), v3.up);
            RotFingerToLocal(FingerName.Pinky, 2, fw_dn(20), v3.up);
