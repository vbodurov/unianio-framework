...
namespace Unianio.Moves
{
    ...
    {
        readonly List<RangeState<T>> _ranges = new List<RangeState<T>>();  
        readonly double _from;
        RangeState<T> _lastRangeState;
        ...
        int GetMiddleIndex(int min, int max)
        {
            if (min < 0) min = 0;
